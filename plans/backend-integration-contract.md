...
A Supabase project is already provisioned and wired into the codebase via autogenerated files — do not edit these manually.

| Field | Value |
|---|---|
| Project ID | `wvqovoktkhaeibsnbfzc` |
| Project URL | `https://wvqovoktkhaeibsnbfzc.supabase.co` |
| Credentials file | `utils/supabase/info.tsx` (exports `projectId` and `publicAnonKey`) |
| Edge function entry | `supabase/functions/server/index.tsx` |
| KV client | `supabase/functions/server/kv_store.tsx` |

When Phase 1 begins, import credentials from `utils/supabase/info.tsx` exclusively. Never hardcode project refs or keys elsewhere.
...
- Supabase is connected but not yet wired to UI data paths; keep `src/data/model.ts` as the seed source until each screen has an equivalent query/mutation adapter.
